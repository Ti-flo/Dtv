...
- [ ] **Formule de brisage des ARMES (anomalie à élucider)** : la formule est validée
  sur armures + armes bien fournies (Hache du Mulou/Brèche : prédit ≈ observé), MAIS
  certaines armes bas niveau rendent 3–5× plus que prédit (captures Flo) :
  Marteau du Boufcoul niv41 (×4.5 **uniforme** sur ses 4 runes → pas du maging),
  Cerberus niv16 (×10), Bâton Feuillu niv14, Pelle à Thart' niv24 (×3). Facteur
  variable par item (ni constante, ni décalage de niveau simple). Hypothèses : floor
  bas niveau, contribution des dégâts d'arme, type d'arme. La capture protocole du
  brisage (type+compte exact des runes) tranchera. En attendant, le brisage
  **sous-estime ces armes** (conservateur).
