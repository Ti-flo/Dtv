...
- [ ] 🔴 **PRIORITAIRE (Flo) — Récursivité des sous-crafts dans le coût de craft** :
  aujourd'hui `craft.craft_plan` chiffre une recette à **un seul niveau** : un
  ingrédient lui-même craftable est pris à son **prix HDV**, pas à son **coût de
  craft** (potentiellement bien moins cher). Il faut résoudre la recette de façon
  **récursive** : pour chaque ingrédient, prendre min(prix HDV, coût de craft de
  l'ingrédient) — avec garde anti-cycle et profondeur max. Impacte le coût affiché
  partout (onglet Craft & Brisage, `dtv craftplan`, futur onglet Bonnes affaires).
  À faire très bientôt (signalé important par Flo, S13).
- [ ] ⭐ **Coût de craft/brisage basé sur nos ACHATS RÉELS (Flo, S13)** : aujourd'hui
  le coût d'un ingrédient = prix HDV (tiers/moyen). À terme, croiser avec
  `transactions_observations.csv` (nos achats : GID, prix unitaire, quantité) pour
  calculer un **coût moyen pondéré de notre stock réel**. Un nouveau tableau craft/brisage
  utiliserait ce coût-là (« combien ça M'a coûté » vs « combien ça coûte au marché »)
  → marge réelle, et savoir si on a sur/sous-payé. Prérequis : stock courant par GID
  (achats − consommés en craft/brisage). Lié à la capture des ventes (PROTOCOL §5-ter).
- [ ] ⭐ **Rapport HTML interactif (Flo, S13)** — `dtv report` génère un HTML autonome
  (données SQLite embarquées en JSON, onglets + tri + graphes). Onglets cibles :
  (1) **Prix dans le temps** par tier (min/max/moyen x1/x10/x100/x1000) ;
  (2) **Ressources achetées** (depuis transactions) + prix payé ;
  (3) **Craft & Brisage** (les 2 tableaux théorique/réel) ;
  (4) **Bonnes affaires du moment** : items dont le prix actuel est très en-dessous de
  leur médiane historique (z-score / écart %), + opportunités craft/brisage induites ;
  (5) idées : dashboard KPIs, prix des runes dans le temps, marges réelles (achats+ventes).
  Reco archi : génération STATIQUE (pas de serveur), self-contained, ouvrable hors-ligne.
  → **EN COURS (S13)** : `dtv report` livré (`collector/report.py` + `_report_template.py`).
  ✅ Onglet (1) Prix dans le temps (tiers, Var j/s/m, volume, favoris/listes, filtres
  type/niveau, graphe SVG) ; ✅ Onglet (3) Craft & Brisage (2 tableaux théo/réel,
  détail au clic, sélecteur de batch, R/moy, réutilise `brisage.build_ranking` +
  `craft.craft_plan`). 🔜 (4) Bonnes affaires, puis (2) Ressources achetées.
...
- [ ] ⭐ **IMPORTANT (Flo) — Auto-collecte du coefficient de brisage via CDP au moment du brisage** :
  ...
  - ✅ **RÉSOLU (dump CDP 27/06)** : le message est `ExchangeCraftResultRunicRecyclingMessage`.
    Champs : `frequencyBonus` = **coefficient réel** (confirmé : 8 vu en jeu = frequencyBonus:8),
    `resultObjects` = runes obtenues (vide si coeff trop bas), `objectGID` = item brisé.
    `PassiveCollector` le parse → écrit `data/raw/brisage_observations.csv` auto. Détail
    protocole : `PROTOCOL.md` §5-bis. (Le HAR DevTools ne capte PAS le WS → c'est le dump
    CDP `--dump-raw` qui a tout révélé.)
...
### Session 13 (plan de craft à tiers, brisage à 2 tableaux, fix runes capture)
- **`dtv craftplan`** : plan d'achat optimisé par **tier HDV** (x1/x10/x100/x1000).
  Choix du tier en 2 temps — **praticabilité d'abord** (≤ `MAX_PURCHASES`=30 achats),
  **meilleur prix unitaire ensuite**, repli sur le plus grand lot si aucun n'est
  praticable. Échelle n_crafts revue (2k–10k → 100 crafts). Colonne **Achats**
  (nb de lots à acheter) + alerte si > 30. **Deux plans affichés** : réaliste (100 %)
  et minimal (10 %, `n_crafts//10`) + ligne de comparaison kamas/craft — pour
  arbitrer « beaucoup d'un seul craft » vs « peu de plusieurs ».
- **Prix de lot réels** (`store.tier_prices_for_gids`) : **dernier snapshot HDV par GID**
  (pas le MIN sur 7 j, qui ramenait de vieux lots bradés), repli `avg_prices` si le
  snapshot est périmé (> `--days`, déf 7 j). La capture logue désormais les 4 tiers.
- **`dtv brisage --craft`** utilise ce coût de craft à **tiers optimisés** (via
  `craft.craft_plan`) au lieu du seul `avg_price_x1`. Repli transparent si base vide.
- **Brisage à DEUX tableaux** (cf. `BRISAGE.md`) : 🏆 **TOP THÉORIQUE** (tout le
  catalogue au coeff `--coeff`, tri Coeff Min = découverte) + 🎯 **BRISAGES RÉELS**
  (uniquement les items déjà brisés, **coeff réel** observé, tri bénéfice réel,
  marqueur ✓/✗ = watchlist du moment). Chaque tableau cohérent dans sa colonne
  bénéfice (un seul coeff par tableau). Observations **chargées d'office** depuis
  `brisage_obs` (plus besoin de `--observations`).
- **Fix capture « gidNone »** : la structure réelle de
  `ExchangeCraftResultRunicRecyclingMessage` (vérifiée sur HAR S7) utilise
  **`objectGid`/`objectQty`** (casse minuscule) pour les runes dans `resultObjects`,
  alors que l'item brisé est `objectGID`/`quantity`. Le code lisait la mauvaise casse
  → rune perdue. Corrigé + extrait dans `parse_recycling_runes` (testé), rend `gid<N>`
  si le GID rune est inconnu. PROTOCOL.md §5-bis corrigé.
- **Ergonomie Windows** : `dtv.bat` + `dtv/__main__.py` (`python -m dtv`). Bypass
  argparse pour `brisage`/`craft` (les flags `--craft --top 50` n'étaient plus avalés).
- Tests : 10/10 (`test_craft.py`), dont `store.brisage_observations` et
  `parse_recycling_runes`.
