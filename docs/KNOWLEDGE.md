...
- [x] ~~🔴 **Récursivité des sous-crafts dans le coût de craft**~~ ✅ **FAIT (S13)** :
  `craft.resolve_craft_unit_costs(recipes, buy_unit)` résout chaque ingrédient au
  **min(prix HDV, coût de craft récursif)** (mémoïsation + garde anti-cycle +
  profondeur max). `craft_plan(..., craft_alt=…)` tranche par ingrédient
  (`method` ∈ {buy, craft}). Branché de façon cohérente sur le rapport HTML
  (onglet Craft & Brisage + modale qui affiche « craft »), `dtv brisage --craft`
  et `dtv craft_plan`. Tests : `test_resolve_recursion`, `test_plan_craft_alt`.
