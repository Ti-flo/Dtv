...
- [ ] ⭐ **IMPORTANT (Flo) — Auto-collecte du coefficient de brisage + date depuis le serveur** : comme les prix moyens, le coefficient réel d'un item et son historique de brisage doivent venir d'un **message protocole** (à identifier dans `script.js` / captures). Une fois trouvé, `coefficient_reel` + `dernier_brisage` se rempliront **tout seuls** (aujourd'hui : `brisage_observations.csv` à la main). Le coeff varie 1 %–4000 %, tend vers 1 % à chaque brisage → la fraîcheur de la date compte.
